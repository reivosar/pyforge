...
def fill_void_bodies_with_claude(test_file_content: str, source_code: str) -> str:
    if "TODO:CLAUDE_FILL" not in test_file_content:
        return test_file_content

    prompt = (
        "A test file has been auto-generated. Some test bodies are marked TODO:CLAUDE_FILL.\n"
        "These are for void/side-effect methods where mock verification cannot be inferred.\n\n"
        "Rules:\n"
        "- Replace each TODO:CLAUDE_FILL comment with real assertions "
        "(mock.assert_called_once_with(...), etc.)\n"
        "- Do NOT change anything else — keep all imports, mock setup, method names exactly as-is\n"
        "- When/Then structure must be preserved\n\n"
        f"## Source file\n{source_code}\n\n"
        f"## Test file (fill TODO:CLAUDE_FILL sections only)\n{test_file_content}\n\n"
...
def main():
    ...
    # fill void method bodies with Claude (minimal LLM call)
    if void_methods:
        info(f"Calling Claude for {len(void_methods)} void method(s) only...")
        content = fill_void_bodies_with_claude(content, source_text)
    else:
