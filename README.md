...
| Usage-based type inference | Infers type from comparisons, method calls, arithmetic, iteration |

### Assertion Inference (priority chain)

1. `pytest.raises(Exc, match=r"...")` — explicit raise branches
2. `assert result is None` — when return is `None`
3. `assert result is True/False` — bool return patterns
